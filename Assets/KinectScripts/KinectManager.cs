	...
	void OnGUI()
    {
		if(kinectInitialized)
		{
			   ...
			   (computeUserMap != UserMapType.None && computeUserMap != UserMapType.RawUserDepth))
	        {
				...
				rectPantalla.texture = usersLblTex;
	        }
...
//						}
	 ...
	 IEnumerator Example()
    {
		...
		byte[] bytes = usersLblTex.EncodeToPNG();
     		System.IO.File.WriteAllBytes("C:/Users/joels/Documents/SavedScreen.png", bytes);
    }
		...
		for(int i = 0; i < jointsCount; i++)
		{
			   ...
			   bodyData.joint[parent].trackingState != KinectInterop.TrackingState.NotTracked)
			{
				...
				if(posParent != Vector2.zero && posJoint != Vector2.zero)
				{
					...
					KinectInterop.DrawLine(aTexture, (int)posParent.x, (int)posParent.y, (int)posJoint.x, (int)posJoint.y, Color.black);
				}
			}
		}
